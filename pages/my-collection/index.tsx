      ...
      <div className={styles.userCollectionOuterContainer}>
        {session?.user.role === "USER" && (
          <div className={styles.userCollectionImagesContainer}>
            {userImages.map((image) => (
              <div
                className={styles.userCollectionImageContainer}
                key={image.id}
              >
                <img
                  src={image.url}
                  className={styles.userCollectionImage}
                  alt={image.alt}
                />
                {favorited(image.id) ? (
                  <svg
        ...
        {userImages && !userImages.length && (
          <div className={styles.noUserCollectionOuterContainer}>
            <div className={styles.noUserCollectionText}>
              You have not saved any images to your collection
            </div>
            <div>
              <button
                onClick={() => routeToHomePage()}
                className={styles.noUserCollectionButton}
              >
                Find images
              </button>
            </div>
